...
def calculate_closeness_scores(adjacency_list: dict[int, dict[int, float]]) -> dict[int, float]:
    ...
    # BOUCLE PRINCIPALE: On calcule la Centralité de Proximité pour chaque nœud (livre)
    for source_id in nodes_in_graph:
        ...
        # 2. Calculer la Farness et le nombre de nœuds accessibles
        total_distance = 0.0
        ...
        if total_distance > 0:
            # Closeness = Nombre de nœuds accessibles / Somme des distances minimales
            # Ceci est la formule pondérée, qui peut donner > 1.0
            closeness = reachable_nodes_count / total_distance 
        
        # Enregistrement du score pour le nœud source
